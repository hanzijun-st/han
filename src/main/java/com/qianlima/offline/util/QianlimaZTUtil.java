...
public class QianlimaZTUtil {
     ...
     */
    public static Map<String, Object> getFields(String contentId, String fieldName, String validateFields,String apiUrl) {
     ...
     *
     * @param apiUrl
     * @param contentId  多个contentid之间使用 ,进行分割
     * @return returnCode 0:推送成功  -1:参数为空  1:data为空  -2:认证失败  -3:向MQ推送失败 -4:未知错误
     */
    public static Map<String, Object> getBlockUrl(String apiUrl, String contentId) {
        Map<String, Object> resultMap = new HashMap<>();
        String returnCode = "500";
        CloseableHttpClient httpClient = getHttpClient();
        RequestConfig requestConfig = RequestConfig.custom().setConnectionRequestTimeout(60000)
        ...
        try {
            ...
            if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                ...
                if (StringUtils.isNotBlank(result)) {
                    ...
                    message = jsonObject.getString("message");
                    log.info("调用中台API获取附件下载链接 contentid：{} 接口返回code:{}", contentId, returnCode);
                }
            } else {
                log.warn("调用中台API获取附件下载链接, http响应失败 contentid：{} httpcode:{}", contentId, httpResponse.getStatusLine().getStatusCode());
            }
            if (httpResponse != null) {
                httpResponse.close();
            }
        } catch (UnsupportedEncodingException e) {
            log.error("调用中台API获取附件下载链接, 异常:{}", e);
