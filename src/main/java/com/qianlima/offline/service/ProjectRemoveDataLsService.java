...
@Service
@Slf4j
public class ProjectRemoveDataLsService {
    ...
    private String SELECT_SQL = "select infoId,infoTitle,infoType,infoPublishTime,areaProvince,areaCity,areaCountry,xmNumber,zhongBiaoUnit,zhaoBiaoUnit,budget,winnerAmount, infoTypeSegment,extractProjName,biddingType from han_ali_offline_data_ls order by infoId asc";

    private String SELECT_FOR_ONE = "select infoId,infoTitle,infoType,infoPublishTime,areaProvince,areaCity,areaCountry,xmNumber,zhongBiaoUnit,zhaoBiaoUnit,budget,winnerAmount, infoTypeSegment,extractProjName,biddingType,code from han_ali_offline_data_ls where oneCode = ? AND infoPublishForData >= ? order by infoId asc";

    private String SELECT_FOR_TWO = "select infoId,infoTitle,infoType,infoPublishTime,areaProvince,areaCity,areaCountry,xmNumber,zhongBiaoUnit,zhaoBiaoUnit,budget,winnerAmount, infoTypeSegment,extractProjName,biddingType,code from han_ali_offline_data_ls where twoCode = ? AND infoPublishForData >= ? order by infoId asc";

    private String SELECT_FOR_THREE = "select infoId,infoTitle,infoType,infoPublishTime,areaProvince,areaCity,areaCountry,xmNumber,zhongBiaoUnit,zhaoBiaoUnit,budget,winnerAmount, infoTypeSegment,extractProjName,biddingType,code from han_ali_offline_data_ls where threeCode = ? AND infoPublishForData >= ? order by infoId asc";

    private String SELECT_FOR_FOUR = "select infoId,infoTitle,infoType,infoPublishTime,areaProvince,areaCity,areaCountry,xmNumber,zhongBiaoUnit,zhaoBiaoUnit,budget,winnerAmount, infoTypeSegment,extractProjName,biddingType,code from han_ali_offline_data_ls where fourCode = ? AND infoPublishForData >= ? order by infoId asc";

    private String SELECT_FOR_FIVE = "select infoId,infoTitle,infoType,infoPublishTime,areaProvince,areaCity,areaCountry,xmNumber,zhongBiaoUnit,zhaoBiaoUnit,budget,winnerAmount, infoTypeSegment,extractProjName,biddingType,code from han_ali_offline_data_ls where fiveCode = ? AND infoPublishForData >= ? order by infoId asc";

    private String SELECT_FOR_SIX = "select infoId,infoTitle,infoType,infoPublishTime,areaProvince,areaCity,areaCountry,xmNumber,zhongBiaoUnit,zhaoBiaoUnit,budget,winnerAmount, infoTypeSegment,extractProjName,biddingType,code from han_ali_offline_data_ls where sixCode = ? AND infoPublishForData >= ? order by infoId asc";
    ...
    public void handleGetTempNum() throws Exception{
    ...
    private synchronized void handleForDate(Map<String, Object> map) throws Exception{
        ...
        String infoType = map.get("infoType") != null ? map.get("infoType").toString() : "";
        ...
        if (flagCodeOne && StringUtils.isNotBlank(xmNumber) && StringUtils.isNotBlank(infoTypeSegment)){
            oneCode = String.valueOf((xmNumber + infoTypeSegment).hashCode());
        }
        ...
        if (flagCodeTwo && StringUtils.isNotBlank(infoTitle) && StringUtils.isNotBlank(infoTypeSegment)){
            twoCode = String.valueOf((infoTitle + infoTypeSegment).hashCode());
        }

        String threeCode = "";
        // D & F & G完全匹配，B & C & E & H & I & J异同
        if (StringUtils.isNotBlank(winnerAmount) && StringUtils.isNotBlank(zhongBiaoUnit) && StringUtils.isNotBlank(infoTypeSegment)){
            threeCode = String.valueOf((winnerAmount + zhongBiaoUnit + infoTypeSegment).hashCode());
        }

        String fourCode = "";
        // C & E & G完全匹配，B & D & F & H & I & J异同
        if (StringUtils.isNotBlank(budget) && StringUtils.isNotBlank(zhaoBiaoUnit) && StringUtils.isNotBlank(infoTypeSegment)){
            fourCode = String.valueOf((budget + zhaoBiaoUnit + infoTypeSegment).hashCode());
        }

        String fiveCode = "";
        // D & E & G完全匹配，B & C & F & H & I & J异同
        if (StringUtils.isNotBlank(winnerAmount) && StringUtils.isNotBlank(zhaoBiaoUnit) && StringUtils.isNotBlank(infoTypeSegment)){
            fiveCode = String.valueOf((winnerAmount + zhaoBiaoUnit + infoTypeSegment).hashCode());
        }

        String sixCode = "";
        // C & F & G完全匹配，B & D & E & H & I & J异同
        if (StringUtils.isNotBlank(budget) && StringUtils.isNotBlank(zhongBiaoUnit) && StringUtils.isNotBlank(infoTypeSegment)){
            sixCode = String.valueOf((budget + zhongBiaoUnit + infoTypeSegment).hashCode());
        }

        boolean flag = true;
        log.info("处理到了infoId：{}, 当前是第num：{} 条", infoId, atomicInteger.addAndGet(1));
        bdJdbcTemplate.update("update han_ali_offline_data_ls set code = ?,infoPublishForData = ? where infoId = ?", infoId, infoPublishForData, infoId);
        String dnpId = "";
                ...
                if (handleMap != null && ! handleMap.isEmpty()){
                    ...
                    for (Map.Entry<String, List<Map<String, Object>>> entry : entries) {
                        ...
                        if (StringUtils.isNotBlank(dnpId)){
                            bdJdbcTemplate.update("update han_ali_offline_data_ls set code2 = ?, code = ? where infoId = ?", "2.1", dnpId, infoId);
                            flag = false;
                            break;
                        }
                    }
            ...
            if (results != null && results.size() > 0){
                ...
                if (handleMap != null && ! handleMap.isEmpty()){
                    ...
                    for (Map.Entry<String, List<Map<String, Object>>> entry : entries) {
                        ...
                        if (StringUtils.isNotBlank(dnpId)){
                            bdJdbcTemplate.update("update han_ali_offline_data_ls set code2 = ?, code = ? where infoId = ?", "2.2", dnpId, infoId);
                            flag = false;
                            break;
                        }
                    }
            ...
            if (results != null && results.size() > 0){
                ...
                if (handleMap != null && ! handleMap.isEmpty()){
                    ...
                    for (Map.Entry<String, List<Map<String, Object>>> entry : entries) {
                        ...
                        if (StringUtils.isNotBlank(dnpId)){
                            bdJdbcTemplate.update("update han_ali_offline_data_ls set code2 = ?, code = ? where infoId = ?", "2.3", dnpId, infoId);
                            flag = false;
                            break;
                        }
                    }
            ...
            if (results != null && results.size() > 0){
                ...
                if (handleMap != null && ! handleMap.isEmpty()){
                    ...
                    for (Map.Entry<String, List<Map<String, Object>>> entry : entries) {
                        ...
                        if (StringUtils.isNotBlank(dnpId)){
                            bdJdbcTemplate.update("update han_ali_offline_data_ls set code2 = ?, code = ? where infoId = ?", "2.4", dnpId, infoId);
                            flag = false;
                            break;
                        }
                    }
            ...
            if (results != null && results.size() > 0){
                ...
                if (handleMap != null && ! handleMap.isEmpty()){
                    ...
                    for (Map.Entry<String, List<Map<String, Object>>> entry : entries) {
                        ...
                        if (StringUtils.isNotBlank(dnpId)){
                            bdJdbcTemplate.update("update han_ali_offline_data_ls set code2 = ?, code = ? where infoId = ?", "2.5", dnpId, infoId);
                            flag = false;
                            break;
                        }
                    }
            ...
            if (results != null && results.size() > 0){
                ...
                if (handleMap != null && ! handleMap.isEmpty()){
                    ...
                    for (Map.Entry<String, List<Map<String, Object>>> entry : entries) {
                        ...
                        if (StringUtils.isNotBlank(dnpId)){
                            bdJdbcTemplate.update("update han_ali_offline_data_ls set code2 = ?, code = ? where infoId = ?", "2.6", dnpId, infoId);
                            flag = false;
                            break;
                        }
                    }
        ...
        List<Map<String, Object>> maps2 = bdJdbcTemplate.queryForList("SELECT infoId from han_ali_offline_data_ls where code = ?", dnpId);
        if (maps2 != null && maps2.size() > 0){
            if (maps2.size() >= 7){
                bdJdbcTemplate.update("update han_ali_offline_data_ls set keywords = ? where code = ?", "1", dnpId);
            }
        }

        bdJdbcTemplate.update("update han_ali_offline_data_ls set oneCode = ?, twoCode = ?, threeCode = ?, fourCode = ?, fiveCode = ?, sixCode = ? where infoId = ?", oneCode, twoCode, threeCode, fourCode, fiveCode, sixCode, infoId);
    }
