...
@Service
@Slf4j
public class CusDataFieldsService {
    ...
    // 数据入库操作
    private static final String INSERT_ZT_RESULT_HXR = "INSERT INTO han_qc_poc_table (task_id,keyword,content_id,title,content, province, city, country, url, baiLian_budget, baiLian_amount_unit," +
    ...
    private Map<String, Object> getResultMapWithZiTi(JSONArray jsonArray, Map<String, Object> resultMap){
        String blBudget = ""; //获取百自提预算金额
        String blZhongbiaoAmount = ""; //获取自提中标金额
        String blZhaoBiaoUnit = ""; //获取自提招标单位
        String blZhongBiaoUnit = ""; //获取自提中标单位
        String blBidder = ""; //获取候选人
        String blAgents = ""; //百炼代理机构
        for (int d = 0; d < jsonArray.size(); d++) {
            JSONObject object = jsonArray.getJSONObject(d);
            // 获取招标预算
            if (null != object.get("extract_budget")) {
        ...
        for (int d = 0; d < jsonArray.size(); d++) {
            ...
            if (StringUtils.isBlank(blZhongBiaoUnit)){
                    ...
                    if (expandField.get("winners") != null) {
                        ...
                        for (JSONObject winner : winners) {
                            if (winner.get("bidderDetails") != null) {
                                ...
                                for (int i = 0; i < bidderDetails.size(); i++) {
                                    if (bidderDetails.get(i).get("bidder") != null) {
                                        blZhongBiaoUnit += bidderDetails.get(i).get("bidder") + "、";
                ...
                if (StringUtils.isNotBlank(blZhongBiaoUnit)){
                    blZhongBiaoUnit = blZhongBiaoUnit.substring(0, blZhongBiaoUnit.length() - 1);
                }
            }
        }
...
//        noticeMQ.setContentid(Long.valueOf("208790280"));
//        Map<String, Object> allFieldsWithBaiLian = cusDataFieldService.getAllFieldsWithBaiLian(noticeMQ, false);
