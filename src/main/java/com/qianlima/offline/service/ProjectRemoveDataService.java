...
public class ProjectRemoveDataService {
    ...
    private String SELECT_SQL = "select infoId,infoTitle,infoType,infoPublishTime,areaProvince,areaCity,areaCountry,xmNumber,zhongBiaoUnit,zhaoBiaoUnit,budget,winnerAmount, infoTypeSegment,extractProjName,biddingType from han_ali_offline_data order by infoId asc";
    ...
    private synchronized void handleForDate(Map<String, Object> map) throws Exception{
        ...
        List<Map<String, Object>> maps2 = bdJdbcTemplate.queryForList("SELECT infoId from han_ali_offline_data where code = ?", dnpId);
        if (maps2 != null && maps2.size() > 0){
            if (maps2.size() >= 7){
                bdJdbcTemplate.update("update han_ali_offline_data set keywords = ? where code = ?", "1", dnpId);
            }
        }
